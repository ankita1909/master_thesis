    ...
    pub fn key_expansion(&mut self)
        {
          ...
          pub fn add_round_key(&mut self, r:u8)
          {
              ...
              for i in 0..4
              {
                  for j in 0..4
                  {
                    let ind = r * Nb * 4 + i * Nb + j;
                    self.state_t[i as usize][j as usize] ^= self.RoundKey[ind as usize];
                  }
              } 
          }
        ...
        pub fn sub_bytes(&mut self)
            {
                ...
                for j in 0..4
                {
                    self.state_t[j][i] = get_sbox_value(self.state_t[j][i]);
                }
                }
            }
        pub fn shift_rows(&mut self)
            ...
            temp           = self.state_t[0][1];
            self.state_t[0][1] = self.state_t[1][1];
            self.state_t[1][1] = self.state_t[2][1];
            self.state_t[2][1] = self.state_t[3][1];
            self.state_t[3][1] = temp;

            // Rotate second row 2 columns to left  
            temp           = self.state_t[0][2];
            self.state_t[0][2] = self.state_t[2][2];
            self.state_t[2][2] = temp;

            temp       = self.state_t[1][2];
            self.state_t[1][2] = self.state_t[3][2];
            self.state_t[3][2] = temp;

            // Rotate third row 3 columns to left
            temp       = self.state_t[0][3];
            self.state_t[0][3] = self.state_t[3][3];
            self.state_t[3][3] = self.state_t[2][3];
            self.state_t[2][3] = self.state_t[1][3];
            self.state_t[1][3] = temp;
            }
        ...
        pub fn mix_columns(&mut self)
            ...
            {  
                t   = self.state_t[i][0];
                Tmp = self.state_t[i][0] ^ self.state_t[i][1] ^ self.state_t[i][2] ^ self.state_t[i][3] ;
                Tm  = self.state_t[i][0] ^ self.state_t[i][1] ; Tm = AES::xtime(Tm);  self.state_t[i][0] ^= Tm ^ Tmp ;
                Tm  = self.state_t[i][1] ^ self.state_t[i][2] ; Tm = AES::xtime(Tm);  self.state_t[i][1] ^= Tm ^ Tmp ;
                Tm  = self.state_t[i][2] ^ self.state_t[i][3] ; Tm = AES::xtime(Tm);  self.state_t[i][2] ^= Tm ^ Tmp ;
                Tm  = self.state_t[i][3] ^ t ;        Tm = AES::xtime(Tm);  self.state_t[i][3] ^= Tm ^ Tmp ;
            }
            }
        ...
        pub fn inv_sub_bytes(&mut self)
        {
          ...
          for i in 0..4
          {
            for j in 0..4
              {
                  self.state_t[j][i] = get_sbox_invert(self.state_t[j][i]);
              }
          }
      }
      ...
      pub fn inv_shift_rows(&mut self)
      {
        ...
        let mut temp=self.state_t[3][1];
        self.state_t[3][1]=self.state_t[2][1];
        self.state_t[2][1]=self.state_t[1][1];
        self.state_t[1][1]=self.state_t[0][1];
        self.state_t[0][1]=temp;
        ...
        temp=self.state_t[0][3];
        self.state_t[0][3]=self.state_t[1][3];
        self.state_t[1][3]=self.state_t[2][3];
        self.state_t[2][3]=self.state_t[3][3];
        self.state_t[3][3]=temp;
      ...
      pub fn inv_cipher(&mut self)
      {
        ...
        AES::key_expansion(self);
        AES::gen_cipher(self);
        hprintln!("lets check the output of first element in 2d array {}",self.state_t[0][0]);
        }

        pub fn AES128_ECB_decrypt(&mut self)
        {
        AES::inv_cipher(self);
        hprintln!("lets check the output of first element after decryption in 2d array {}",self.state_t[0][0]);
        }
...
pub fn test_aes()
{
    ...
    let mut inp:[[u8;4];4]  = [[0xae,0x2d,0x8a,0x57],[0x1e,0x03,0xac,0x9c],[0x9e,0xb7,0x6f,0xac],[0x45,0xaf,0x8e,0x51]];
    let out:[[u8;4];4] =[[0xf5, 0xd3, 0xd5, 0x85], [0x03, 0xb9, 0x69, 0x9d], [0xe7, 0x85, 0x89, 0x5a], [0x96, 0xfd, 0xba, 0xaf]];
    let mut buffer: [[u8;4];4] = [[0;4];4];
    buffer.copy_from_slice(&inp);
    let mut test_1 = AES {   state_t : &mut buffer,
        RoundKey:[0;176],
        Key: &key
    };
        
    test_1.AES128_ECB_encrypt();
    test_1.AES128_ECB_decrypt();
    assert_eq!(buffer,inp);
    }
}