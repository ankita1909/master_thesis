...
fn get_sbox_invert(num:u8) -> u8
  {
    ...
    pub fn key_expansion(Key: &[u8], RoundKey: &mut [u8])
        {
            let mut i:u8 = 0;
            let mut j:u8 = 0;
            let mut k:u8 = 0;
          ...
          pub fn add_round_key( state_t: &mut [u8], RoundKey: &[u8], r:u8)
          {
              let mut i:u8;
              let mut j:u8;
              for i in 0..16
              {
                    let ind = r * NB * 4 + i;
                    state_t[i as usize]^= RoundKey[ind as usize];                 
              } 
          }

        pub fn sub_bytes(state_t: &mut [u8])
            {
                for i in 0..16
                {         
                     state_t[i] = get_sbox_value( state_t[i]);
                
                }
            }

        pub fn shift_rows(state_t: &mut [u8])
            {
                /*
                0   1   2   3
                4   5   6   7
        ...
        pub fn mix_columns(state_t: &mut [u8])
            {
            ...
            {  
                t   =  state_t[i];
                tmp =  state_t[i] ^  state_t[i+1] ^  state_t[i+2] ^  state_t[i+3] ;
                tm  =  state_t[i] ^  state_t[i+1] ; 
                tm =  xtime(tm);  
                 state_t[i] ^= tm ^ tmp ;
                tm  =  state_t[i+1] ^  state_t[i+2] ; 
                tm =  xtime(tm);  
                 state_t[i+1] ^= tm ^ tmp ;
                tm  =  state_t[i+2] ^  state_t[i+3] ; 
                tm =  xtime(tm);  
                 state_t[i+2] ^= tm ^ tmp ;
                tm  =  state_t[i+3] ^ t ; 
                tm =  xtime(tm);  
        ...
        pub fn gen_cipher(state: &mut [u8], Key: &[u8])
        {
            // key_expansion(self);
            ...
            for no_round in 1..total_number_rounds
            {
                 ...
                 shift_rows(state);
                 mix_columns(state);
                 add_round_key(state, Key, no_round);
            }

             sub_bytes(state);
             shift_rows(state);
             add_round_key(state, Key, total_number_rounds);
...
/*
        pub fn inv_sub_bytes(&mut self){
            ...
            for i in 0..16
            {
                 ...
                 state_t[i] = get_sbox_invert( state_t[i]);
        ...
        pub fn inv_mix_columns(&mut self)
        {
            ...
            { 
              ...
              let d =  state_t[i+3];
               ...
               state_t[i+0] =  multiply(a, 0x0e) ^  multiply(b, 0x0b) ^  multiply(c, 0x0d) ^  multiply(d, 0x09);
               state_t[i+1] =  multiply(a, 0x09) ^  multiply(b, 0x0e) ^  multiply(c, 0x0b) ^  multiply(d, 0x0d);
               state_t[i+2] =  multiply(a, 0x0d) ^  multiply(b, 0x09) ^  multiply(c, 0x0e) ^  multiply(d, 0x0b);
               state_t[i+3] =  multiply(a, 0x0b) ^  multiply(b, 0x0d) ^  multiply(c, 0x09) ^  multiply(d, 0x0e);
        ...
        {
             add_round_key(self, total_number_rounds);   

            for round in (1 .. total_number_rounds).rev()
                {
                     inv_shift_rows(self);
                     inv_sub_bytes(self);
                     add_round_key(self, round);
                     inv_mix_columns(self);
                }

             inv_shift_rows(self);
             inv_sub_bytes(self);
             add_round_key(self, 0);
...
pub fn test_aes()
{
    ...
    let mut state_t: [u8;16] = [0;16];
    state_t.copy_from_slice(&inp);
    let mut RoundKey = [0u8; 176];
    
    key_expansion(&key, &mut RoundKey);
    gen_cipher(&mut state_t, &RoundKey);

    assert_eq!(state_t, out);
